...
class Model:
    ...
    def complete_run(
        self, initialgenefitness, maxiteration=1, epsilon=None, silent=False
    ):
        ...
        initialorganismfitness = compute_all_organism_fitness(
            self.population.data, genefitness
        )
        if not silent:
            print("initial organism fitness = ", initialorganismfitness)
        initialorgfitnessrange = max(initialorganismfitness) - min(
            initialorganismfitness
        )
        if max(initialorganismfitness) == 0:
            raise ValueError("All organism fitness values are 0.")
        elif (
            initialorgfitnessrange == 0
        ):  # if all fitness values are equal take max instead
...
class Strategies:
    ...
    def __init__(
        ...
        mixinglist=None,
        initialgenemixing=None,
        initialorgmixing=None,
    ):
...
class Population:
    ...
    def __init__(self, rawdata, gvfitnessrule, silent=False):
        ...
        for j in range(0, self._m):
            ...
            # fitness functions
            self._populationdata[:, j] = compute_gene_variant_fitness(
                self._rawdata[:, j], self._gvfitnessrules[j]
            )
        if not silent:
...
def compute_gene_variant_fitness(genedata, gvfrule):
    ...
    """
    if (
        gvfrule == GVRule.PERCENTAGE_INVERTED
        or gvfrule == GVRule.PERCENTAGE_INVERTED_CAPPED
    ):
        invert = True
    else:
        invert = False
    ...
    if (
        gvfrule == GVRule.PERCENTAGE_CAPPED
        or gvfrule == GVRule.PERCENTAGE_INVERTED_CAPPED
    ):
        cap = True
    else:
    ...
    for i in range(0, n):
        if invert:
            if cap:
                gvfitness[i] = (
                    (maxvariant - genedata[i]) / (maxvariant - minvariant)
                    if (maxvariant - minvariant) != 0
                    else 0
                )
            else:
                gvfitness[i] = (
                    (maxvariant - genedata[i]) / maxvariant if maxvariant > 0 else 0
                )
        else:
            if cap:
                gvfitness[i] = (
                    genedata[i] / (maxvariant - minvariant) if maxvariant > 0 else 0
                )
            else:
...
def compute_gene_deltas(
    ...
    if strategies.gsstrategy == GSStrategy.DOMINANT:
        ...
        deltaG = (
            4
            / float(n)
            * genefitness[geneindex]
            * genefitness[geneindex]
            * (genevariantfitness - 1 / 2)
        )
        ...
        for j in range(0, m):
            ...
            if orgfitness[orgindex] == 0:
                ...
                deltaO[j] = (
                    -2
                    / n
                    * (genecontribution / orgfitness[orgindex] - 1 / m)
                    * orgfitness[orgindex]
                )
            ...
            for j in range(0, m):
                ...
                if orgfitness[orgindex] == 0:
                    ...
                    deltaO[j] += (
                        ...
                        * (genecontribution / orgfitness[orgindex] - 1 / m)
                        * (1 / nrel)
                        * (orgfitness[orgindex] - orgfitness[rel])
                        / initialorgfitnessrange
                    )
...
def iteration(
    ...
    for i in range(0, n):
        for s, strat in enumerate(strategies.mixinglist):
            deltaOstmp[i, :, s] = compute_organism_deltas(
                i,
                population,
                genefitness,
                orgfitness,
        ...
        for j in range(0, m):
            for s, strat in enumerate(strategies.mixinglist):
                deltaGstmp[i, j, s] = compute_gene_deltas(
                    j,
                    population[i, j],
                    population[:, j],
                    population[i, :],
...
def compute_gene_similarity(population):
    ...
    for i in range(0, m):
        for j in range(0, m):
            tmp = np.linalg.norm(population[:, i] - population[:, j])
            genediversity[i, j] += tmp
