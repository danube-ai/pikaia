...
class TestGeneticLayer:
    ...
    def test_different_genes_shapes(self):
        ...
        for genes in [1, 8, 16, 32]:
            layer = GeneticLayer(64, 32, genes)
            x = torch.randn(5, 64)
            y = layer(x)
            assert y.shape == (5, 32)
